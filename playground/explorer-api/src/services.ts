...
import { upsertBlock, upsertTx, getAddressSummary, getLatestBlocks, getLatestTxs, getAddressTxs } from './db.js';
...
export class SearchService {
  ...
  async search(q: string) {
    ...
    if (q.startsWith('0x') && q.length === 66) {
      ...
      if (res) return { type: 'tx', value: res };
    }
    // address
    if (q.startsWith('0x') && q.length === 42) {
      const code = await this.rpc.call<string>('eth_getCode', [q, 'latest']);
      return { type: 'address', value: { address: q, isContract: code && code !== '0x' } };
    }
    // block number
    const n = Number(q);
    if (!Number.isNaN(n)) {
      const block = await this.blocks.getBlock(q);
      if (block) return { type: 'block', value: block };
    }
...
export class TraceService {
  ...
  async trace(hash: string, opts: { mode?: 'tree' | 'steps'; memory?: boolean; stack?: boolean }) {
    if (opts.mode === 'tree') {
      ...
      return this.rpc.debugTraceTransaction(hash, { tracer: 'callTracer' });
    }
    ...
    return this.rpc.debugTraceTransaction(hash, config);
  }

  async gasReport(hash: string, decodeFn: (addr: string, input: string) => Promise<string>) {
    const tree = await this.rpc.debugTraceTransaction(hash, { tracer: 'callTracer' });
